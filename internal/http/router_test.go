...
func TestHealthz(t *testing.T) {
	for _, path := range []string{"/healthz", "/health", "/api/healthz"} {
		t.Run(path, func(t *testing.T) {
			...
			if got := rec.Header().Get("Content-Type"); got != "application/json" {
				t.Fatalf("path %s: Content-Type = %q, want application/json", path, got)
			}
			var body map[string]string
			if err := json.NewDecoder(rec.Body).Decode(&body); err != nil {
				t.Fatalf("path %s: Decode body error = %v", path, err)
			}
			if body["status"] != "ok" {
				t.Fatalf("path %s: status body = %q, want ok", path, body["status"])
			}
		})
	}
}
